...
func GetListById(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	query := `SELECT * FROM get_list_by_id($1, $2)`
	err = conn.QueryRow(context.Background(), query, list_id, user_id).Scan(
		&list.Id,
		&list.Title,
		&list.Total_likes,
		&list.Total_books,
